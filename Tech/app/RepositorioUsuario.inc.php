...
class RepositorioUsuario
{
    ...
    public static function obtener_numero_usuarios($conexion)
    {
        ...
        if(isset($conexion))
        {
            ...
            {
                include_once 'Usuario.inc.php';
                $sql = "SELECT COUNT(*) AS total FROM tech.usuarios";
                $sentencia = $conexion -> prepare($sql);
                $sentencia -> execute();
    ...
    public static function nombre_existe($conexion, $nombre)
    {
        ...
        if(isset($conexion))
        {
            ...
            {
                include_once 'Usuario.inc.php';
            $sql = "SELECT * FROM tech.usuarios WHERE nombre = ':nombre'";
            $sentencia = $conexion -> prepare($sql);
            $sentencia -> bindParam(':nombre', $nombre, PDO::PARAM_STR);
            $sentencia -> execute();
            $resultado = $sentencia -> fetchAll();
    ...
    public static function email_existe($conexion, $email)
    {
        ...
        if(isset($conexion))
        {
            ...
            {
                include_once 'Usuario.inc.php';
            $sql = "SELECT * FROM tech.usuarios WHERE email = ':email'";
            $sentencia = $conexion -> prepare($sql);
            $sentencia -> bindParam(':email', $email, PDO::PARAM_STR);
            $sentencia -> execute();
            $resultado = $sentencia -> fetchAll();
           ...
           try {
               ...
               if(!empty($resultado))
               {
                   $usuario = new Usuario($resultado['id'],
                           $resultado['nombre'],$resultado['email'],
